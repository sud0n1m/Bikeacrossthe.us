...
function main() {
	...
	$fe_key = 'GY7ojA7BQOXv';
	$fe_secret = 'TEjM3WqNa9ETZRdhDJRBekXxj7agVCaz';
	...
	if (@$_GET['f'] == 'start') {
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			case 'Move!':
				...
				exit;
			case 'Lookup':
				echo "<p>Lookup results:</p><div><code>".nl2br(htmlspecialchars(var_export($fe->lookup($where), 
TRUE)))."</code></div>";
				break;
			}
		}
		
		?><p>You are authenticated with <a href="<?php print htmlspecialchars(FireEagle::$FE_ROOT) ?>">Fire Eagle</a>!  (<a 
href="?f=start">Change settings</a>.)</p><?php
		...
		if (TRUE || $_SESSION['can_write']) { // fix when we get 'writable' from the 'user' response
			...
			<form method="POST">
			<p><label for="free-text-entry">Free-text entry:</label> <input type="text" name="q" id="free-text-entry" 
size="40"></p>
			<p><label for="place-id">Place ID:</label> <input type="text" name="place_id" id="place-id" size="40"></p>
			<p><label for="lat">Lat:</label> <input type="text" name="lat" id="lat" size="10"> <label for="lon">Lon:</label> <input 
type="text" name="lon" size="10"></p>
		...
		?><p><a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>?f=start">Click here to authenticate with Fire 
Eagle!</a></p><?php
	}
}
